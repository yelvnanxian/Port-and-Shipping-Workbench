...
tracking.getRange('A1:I16').values = [
  ['船司', '到港时间', '提单号', '柜号', '卸船时间', '船只状态', '最后更新时间', '备注', '进度'],
  ...records,
];
tracking.getRange('A1:I1').format = { fill: '#0B3347', font: { bold: true, color: '#FFFFFF', size: 11 }, verticalAlignment: 'center', horizontalAlignment: 'center', borders: { preset: 'outside', style: 'thin', color: '#0B3347' } };
tracking.getRange('A1:I1').format.rowHeight = 30;
tracking.getRange('A2:I16').format = { font: { color: '#243D48', size: 10 }, verticalAlignment: 'center', borders: { insideHorizontal: { style: 'thin', color: '#DCE5E5' } } };
tracking.getRange('A2:I16').format.rowHeight = 30;
for (let row = 2; row <= 16; row += 2) tracking.getRange(`A${row}:I${row}`).format.fill = '#F3F8F7';
...
for (const [column, width] of Object.entries({ A: 16, B: 20, C: 20, D: 18, E: 20, F: 18, G: 20, H: 36, I: 12 })) tracking.getRange(`${column}:${column}`).format.columnWidth = width;
tracking.getRange('H2:H200').format.wrapText = true;
tracking.tables.add('A1:I16', true, 'ShipmentTrackingTable').style = 'TableStyleMedium2';
...
const preview = await workbook.render({ sheetName: '船期追踪', range: 'A1:I16', scale: 1.4, format: 'png' });
