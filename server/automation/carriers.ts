...
export const CARRIER_RULES: CarrierRule[] = [
  ...
  { prefix: 'MAEU', code: 'MAERSK', name: '马士基', removePrefix: true, queryMode: 'bill', url: 'https://www.maersk.com/tracking/', integration: 'ready', integrationMessage: '已接入官网浏览器查询与完整多港事件解析；核验提单号和柜号并保留官网当地时间，Akamai 拒绝会明确标记为风控而非订单号无效' },
  ...
  { prefix: 'WHLC', code: 'WANHAI', name: '万海', removePrefix: true, queryMode: 'bill-or-container', url: 'https://cn.wanhai.com/cec/#/cargotracking?q=N', integration: 'limited', integrationMessage: '已接入 Patchright 持久会话，避免旧通用浏览器纯白页；提单无结果后按 OR 规则自动查询柜号，并保留完整页面、接口原文和截图证据' },
  { prefix: 'ZIMU', code: 'ZIM', name: '以星', removePrefix: false, queryMode: 'bill-and-container', url: 'https://www.zimchina.com/tools/track-a-shipment', integration: 'limited', integrationMessage: '已接入 Patchright 持久会话，分别查询提单号和柜号并合并；查询期间禁用地图自动滚动，安全验证页保持等待且不重复刷新' },
  ...
  { prefix: 'SML', code: 'SMLINE', name: '森罗', removePrefix: true, queryMode: 'bill-or-container', url: 'https://esvc.smlines.com/smline/CUP_HOM_3301.do?sessLocale=zh', integration: 'ready', integrationMessage: '提单号去除 SMLM 前缀后与柜号分别查询；任一路成功即可采用，两路成功时合并结果' },
