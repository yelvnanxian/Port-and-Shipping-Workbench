...
查询顺序统一为“提单号优先”：提单号得到可核验结果后立即结束本条查询，不再重复请求柜号；只有官网明确返回无结果，且记录提供柜号时，才会再查询一次柜号。验证码、风控、超时、网络错误和解析异常不会被误判为提单无效，也不会自动触发柜号请求。马士基仍保留去前缀提单号、完整提单号、柜号的专用兼容顺序；赫伯罗特按官网要求使用完整柜号的普通浏览器采集流程。
...
- 证据截图/接口凭证默认保留 7 天；同一船司的同一提单号或柜号只保留最新一份。
- 预览证据清理：`npm run cleanup:evidence`；确认后执行：`npm run cleanup:evidence -- --apply`。
- 系统设置中的“清理自动化 Chrome”只清理工作台启动的浏览器进程，不删除人工验证 Cookie。
- 清理浏览器缓存前必须停止查询任务；不要手动删除整个 `data/browser-profile/` 目录，否则可能丢失验证会话。
