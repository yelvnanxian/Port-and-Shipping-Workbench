...
- 工作台固定使用真实官网/官方接口。只有官方查询成功并解析出字段时才写入时间；未完成联调、验证码、接口错误或格式异常都会明确写入“失败”和原因，不会伪造结果。
...
工作台固定使用官网模式，不会展示或写入虚构结果。`OOLU` 单号会走 OOCL 官方查询解析器，`HDUJ` 单号会走合德官方 HTML 解析器；其余尚未完成联调的船司会得到清晰的“官网解析器待联调”错误。若官网返回 Cloudflare/验证页面、官方服务错误或字段不完整，工作台会保留失败原因，不会写入假时间。
