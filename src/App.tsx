...
type TableViewMode = 'operation' | 'full';
        ...
        <div className="content">
          ...
          <section className={`table-card ${denseTable ? 'compact-table' : ''}`}>
            <div className="table-header">
              <div><h2>船期追踪</h2><span>共 {filtered.length} 条记录 · 勾选后可只更新指定船期</span><span className="tracking-table-hint">当前为{tableViewMode === 'operation' ? '操作视图，点击展开详情查看港口与备注' : '完整视图，显示全部字段'}</span></div>
...
function ModulePage({ page, data, automation, authEnabled, currentUser, syncing, onSync, onMark, onDelete, onToggleAutomation, onCreateBackup, onRestoreBackup, onAutomationUpdated, onUpload, onToast, onOpenDetail, onOpenEdit, onOpenManual }: {
  ...
  const [trackingCarrier, setTrackingCarrier] = useState('全部船司');
  const [trackingStatus, setTrackingStatus] = useState<'全部状态' | ShipmentStatus>('全部状态');
  ...
  useEffect(() => {
    ...
    setSelectedTasks((previous) => new Set([...previous].filter((id) => existing.has(id))));
  }, [tasks]);
  ...
  const filteredTrackingShipments = useMemo(() => {
    const normalized = trackingQuery.trim().toLowerCase();
    return trackingShipments.filter((item) => {
      const matchesQuery = !normalized || [item.billNo, item.containerNo, item.carrier, item.carrierCode]
        .some((field) => field.toLowerCase().includes(normalized));
      const matchesCarrier = trackingCarrier === '全部船司' || item.carrierCode === trackingCarrier;
      const matchesStatus = trackingStatus === '全部状态' || item.status === trackingStatus;
      return matchesQuery && matchesCarrier && matchesStatus;
    });
  }, [trackingShipments, trackingQuery, trackingCarrier, trackingStatus]);
  const allTrackingSelected = filteredTrackingShipments.length > 0 && filteredTrackingShipments.every((shipment) => selectedShipments.has(shipment.id));
  ...
  return <div className="module-page">
    ...
    {moduleLoading && <div className="module-loading"><LoaderCircle className="spin" />正在加载模块数据…</div>}
      ...
      <div className="module-card-header"><div><strong>全部追踪记录</strong><span>Excel 当前共 {trackingShipments.length} 条 · 可筛选、查询并单独或批量更新船期</span></div><div className="table-header-actions"><div className="tracking-view-switch" role="group" aria-label="追踪表显示模式"><button className={trackingViewMode === 'operation' ? 'active' : ''} onClick={() => setTrackingViewMode('operation')}>操作视图</button><button className={trackingViewMode === 'full' ? 'active' : ''} onClick={() => setTrackingViewMode('full')}>完整视图</button></div>{selectedShipments.size > 0 && <><button className="secondary-button compact-button" onClick={() => onSync({ shipmentIds: [...selectedShipments] })} disabled={syncing}><RefreshCw size={14} />更新已选 ({selectedShipments.size})</button><button className="danger-button" onClick={() => onDelete([...selectedShipments])} disabled={syncing}><Trash2 size={13} />删除已选</button></>}<div className="compact-legend"><span className="legend-dot success" />已完成卸船<span className="legend-dot info" />等待卸船<span className="legend-dot muted-dot" />等待到港</div></div></div>
      <div className="tracking-module-toolbar"><label className="search-box"><Search size={16} /><input value={trackingQuery} onChange={(event) => setTrackingQuery(event.target.value)} placeholder="搜索提单号、柜号或船司" />{trackingQuery && <button onClick={() => setTrackingQuery('')}><X size={13} /></button>}</label><div className="select-wrap"><select value={trackingCarrier} onChange={(event) => setTrackingCarrier(event.target.value)}><option value="全部船司">全部船司</option>{Array.from(new Set(trackingShipments.map((item) => item.carrierCode))).map((code) => <option key={code} value={code}>{carrierLabel(code)}</option>)}</select><ChevronDown size={14} /></div><div className="select-wrap"><select value={trackingStatus} onChange={(event) => setTrackingStatus(event.target.value as typeof trackingStatus)}>{statusOptions.map((item) => <option key={item} value={item}>{item}</option>)}</select><ChevronDown size={14} /></div>{(trackingQuery || trackingCarrier !== '全部船司' || trackingStatus !== '全部状态') && <button className="clear-filter" onClick={() => { setTrackingQuery(''); setTrackingCarrier('全部船司'); setTrackingStatus('全部状态'); }}>清除筛选</button>}<span className="tracking-filter-count">显示 {filteredTrackingShipments.length} / {trackingShipments.length} 条</span></div>
      <div className="module-table-wrap"><table className={`module-table module-tracking-table ${trackingViewMode}-view`}><thead><tr><th className="check-col"><input type="checkbox" checked={allTrackingSelected} onChange={() => setSelectedShipments(allTrackingSelected ? new Set([...selectedShipments].filter((id) => !filteredTrackingShipments.some((shipment) => shipment.id === id))) : new Set([...selectedShipments, ...filteredTrackingShipments.map((shipment) => shipment.id)]))} /></th><th>船司</th><th>提单号</th><th>柜号</th><th className="tracking-secondary-column">当前港口</th><th className="tracking-secondary-column">预计到达港口</th><th className="tracking-secondary-column">预计到达时间</th><th>到港时间</th><th>卸船时间</th><th>船只状态</th><th>人工标记</th><th>进度</th><th className="tracking-secondary-column">最后更新</th><th className="tracking-secondary-column">真实性核验</th><th className="module-tracking-actions">操作</th></tr></thead><tbody>
        {filteredTrackingShipments.map((item) => {
...
function RunHistory({ runs, selected, onToggle, onSelectAll, onDelete, onDeleteSelected }: { runs: RunView[]; selected: Set<string>; onToggle: (id: string) => void; onSelectAll: (ids: string[], checked: boolean) => void; onDelete: (id: string) => void; onDeleteSelected: () => Promise<void> }) {
  const [expanded, setExpanded] = useState<Set<string>>(new Set());
  const allSelected = runs.length > 0 && runs.every((run) => selected.has(run.id));
  const partiallySelected = !allSelected && runs.some((run) => selected.has(run.id));
  function toggleAllRuns(checked: boolean) {
    onSelectAll(runs.map((run) => run.id), checked);
  }
...
function MetricListModal({ metricKey, shipments, syncing, onClose, onExport, onMark, onSync, onDelete, onEdit, onDetail }: {
  ...
  const [query, setQuery] = useState('');
  const [carrier, setCarrier] = useState('全部船司');
  const [status, setStatus] = useState<'全部状态' | ShipmentStatus>('全部状态');
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const filtered = useMemo(() => {
    const normalized = query.trim().toLowerCase();
    const matches = shipments.filter((item) => {
      const matchesQuery = !normalized || [item.billNo, item.containerNo, item.carrier, item.carrierCode]
        .some((field) => field.toLowerCase().includes(normalized));
      const matchesCarrier = carrier === '全部船司' || item.carrierCode === carrier;
      const matchesStatus = status === '全部状态' || item.status === status;
    ...
    return filterAndSortShipments(matches, dateField, dateFrom, dateTo, timeSort);
  }, [shipments, dateField, dateFrom, dateTo, timeSort, query, carrier, status]);
  const availableCarriers = useMemo(() => Array.from(new Set(shipments.map((item) => item.carrierCode))), [shipments]);
  ...
  return <div className="modal-backdrop metric-list-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="metric-list-modal" role="dialog" aria-modal="true" aria-labelledby="metric-list-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="metric-list-toolbar">
        <label><span>查询</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="提单号、柜号或船司" /></label>
        <label><span>船司</span><select value={carrier} onChange={(event) => setCarrier(event.target.value)}><option value="全部船司">全部船司</option>{availableCarriers.map((code) => <option key={code} value={code}>{carrierLabel(code)}</option>)}</select></label>
        <label><span>船只状态</span><select value={status} onChange={(event) => setStatus(event.target.value as typeof status)}>{statusOptions.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
        ...
        <button className="secondary-button metric-reset" onClick={() => { setQuery(''); setCarrier('全部船司'); setStatus('全部状态'); setDateFrom(''); setDateTo(''); setTimeSort('default'); }}>重置</button>
      </div>
      ...
      <div className="metric-list-table"><table><thead><tr><th className="check-col"><input type="checkbox" checked={allSelected} onChange={() => setSelected(allSelected ? new Set() : new Set(filtered.map((shipment) => shipment.id)))} /></th><th>船司</th><th>提单号 / 柜号</th><th>当前港口</th><th>预计到达港口</th><th>预计到达时间</th><th>到港时间</th><th>卸船时间</th><th>船只状态</th><th>人工标记</th><th>最后更新</th><th>操作</th></tr></thead><tbody>{filtered.length ? filtered.map((item) => <tr key={item.id} className={`${selected.has(item.id) ? 'selected-row' : ''} ${item.manualMark === '已清关' ? 'cleared-row' : ''}`}><td className="check-col"><input type="checkbox" checked={selected.has(item.id)} onChange={() => toggleOne(item.id)} /></td><td><div className="carrier-cell"><CarrierMark code={item.carrierCode} /><div><strong>{carrierLabel(item.carrierCode, item.carrier)}</strong><span>{item.carrierCode}</span></div></div></td><td><div className="stacked"><strong className="mono">{item.billNo}</strong><span className="mono muted">{item.containerNo || '—'}</span></div></td><td>{item.trackingDetail?.currentPort || '—'}</td><td>{item.trackingDetail?.estimatedArrivalPort || '—'}</td><td>{item.trackingDetail?.estimatedArrivalTimeText || '—'}</td><td><div className="date-cell eta">{formatDateTime(item.eta, true)}</div></td><td><div className="date-cell discharge">{formatDateTime(item.dischargeTime, true)}</div></td><td><VesselStateBadge shipment={item} /></td><td><ManualMarkSelect value={item.manualMark} onChange={(value) => onMark(item.id, value)} disabled={syncing} /></td><td><div className="update-cell"><span>{timeAgo(item.lastUpdated)}</span><small>{formatDateTime(item.lastUpdated)}</small></div></td><td><div className="row-actions"><button className="row-action" title="人工修改时间与状态" onClick={() => { onClose(); onEdit(item); }} disabled={syncing}><Pencil size={14} /></button><button className="row-action" title="只更新这一条" onClick={() => onSync([item.id])} disabled={syncing || item.manualMark === '已清关'}><RefreshCw size={14} /></button><button className="row-action danger-action" title="删除这条记录" onClick={() => onDelete([item.id])} disabled={syncing}><Trash2 size={14} /></button><button className="row-action" title="查看详情" onClick={() => { onClose(); onDetail(item); }}><ChevronRight size={17} /></button></div></td></tr>) : <tr><td colSpan={12}><div className="empty-state"><Search size={23} /><strong>当前条件下没有记录</strong><span>调整日期范围后再试</span></div></td></tr>}</tbody></table></div>
    </section>
  </div>;
}
...
function RouteTimeline({ route, detail, shipment, onManualMark }: { route?: string | null; detail?: Shipment['trackingDetail']; shipment?: Shipment; onManualMark?: (id: string, value: ManualMark) => Promise<void> }) {
  const stops = detail?.routeStops?.length
    ? detail.routeStops
    : (route || '').split(/\s*→\s*/).map((name) => ({ name: name.trim(), role: 'unknown' as const })).filter((stop) => stop.name);
  if (!stops.length && !shipment) return null;
  ...
  const shipmentArrivalKind = shipment ? arrivalKindFor(shipment) : null;
  ...
  const summaryFacts = [
    ...
    ['官网查询号', shipment?.verificationNo || shipment?.billNo || ''],
    ['查询进度', shipment ? <ProgressBadge shipment={shipment} /> : ''],
    ['船只状态', shipment ? <VesselStateBadge shipment={shipment} /> : ''],
    ['人工标记', shipment && onManualMark ? <ManualMarkSelect value={shipment.manualMark} onChange={(value) => onManualMark(shipment.id, value)} /> : ''],
    ['当前港口', detail?.currentPort || ''],
    ['预计到达港口', detail?.estimatedArrivalPort || ''],
    ['预计到达时间', detail?.estimatedArrivalTimeText || ''],
    // ETA 已经由官网完整线路字段展示时，不再在同一抽屉重复显示一遍；
    // 只有实际 ATA 或没有结构化 ETA 时才补充工作簿字段。
    [shipmentArrivalKind === 'ATA' ? '实际到港时间（ATA）' : '预计到达时间（ETA）', shipment?.eta && (shipmentArrivalKind === 'ATA' || !detail?.estimatedArrivalTimeText) ? formatDateTime(shipment.eta) : ''],
    ['卸船时间', shipment?.dischargeTime || (shipment?.vesselState === '已到港已卸船' ? '已确认完成，官网未提供精确时间' : '')],
  ].filter(([, value]) => Boolean(value));
  const summaryLabels = new Set(summaryFacts.map(([label]) => label));
  const routeFactLabels = new Set(['提单号', '柜号', '装货港', '卸货港', '预计到港时间', '预计到达时间']);
  const facts = [...summaryFacts.map(([label, value]) => ({ label, value })), ...(detail?.facts || []).filter((fact) => !summaryLabels.has(fact.label) && !routeFactLabels.has(fact.label))];
  return <section className="route-timeline"><div className="route-timeline-heading"><MapPin size={17} /><div><strong>官网完整运行线路</strong><span>{detail ? `已采集 ${detail.events.length} 条官网轨迹事件` : '按船司官网轨迹整理'}</span></div></div>{facts.length ? <div className="tracking-facts">{facts.map((fact) => <div key={`${fact.label}-${String(fact.value)}`}><span>{fact.label}</span><strong>{fact.value}</strong></div>)}</div> : null}{stops.length ? <div className="route-stops">{stops.map((stop, index) => <div className={`route-stop ${index === stops.length - 1 && !unassignedEvents.length ? 'last' : ''}`} key={`${stop.name}-${index}`}><span className="route-stop-dot"><MapPin size={11} /></span><div className="route-stop-content"><strong>{stop.name}</strong><span>{roleLabels[stop.role] || roleLabels.unknown}</span>{eventsFor(index).map(renderEvent)}</div></div>)}{unassignedEvents.length > 0 && <div className="route-stop last"><span className="route-stop-dot"><MapPin size={11} /></span><div className="route-stop-content"><strong>其他官网轨迹事件</strong><span>未能匹配到线路节点，保留原始地点</span>{unassignedEvents.map(renderEvent)}</div></div>}</div> : <div className="route-empty">暂无结构化线路节点，已保留官网原始证据。</div>}</section>;
}
