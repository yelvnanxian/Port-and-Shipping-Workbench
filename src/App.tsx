...
export default function App() {
  ...
  async function load(endpoint = '/api/dashboard') {
    const response = await fetch(endpoint, { method: endpoint.includes('sync') ? 'POST' : 'GET' });
    if (!response.ok) throw new Error('无法读取采集数据');
  ...
  async function loadAutomation() {
    ...
    if (!response.ok) throw new Error('无法读取自动化配置');
  ...
  async function handleUpload(file?: File) {
    ...
    setSyncing(true);
    try {
      const response = await fetch('/api/workbooks/upload', { method: 'POST', body: form });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.message || 'Excel 导入失败');
      setData(payload.dashboard);
      setAutomation(payload.automation);
      setSelected(new Set());
  ...
  async function handleIntake() {
    ...
    try {
      const response = await fetch('/api/intake', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ entries }) });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.message || '单号添加失败');
      const runResponse = await fetch('/api/automation/run', { method: 'POST' });
      const runPayload = await runResponse.json();
      if (!runResponse.ok) throw new Error(runPayload.message || '单号查询失败');
    ...
    <div className="app-shell">
      <aside className={`sidebar ${mobileNav ? 'mobile-open' : ''}`}>
        ...
        <div className="sidebar-foot">
          ...
          <div className="user-card"><div className="avatar">林</div><div><strong>林晓航</strong><span>运营管理员</span></div><MoreHorizontal size={17} /></div>
        </div>
      </aside>
        ...
        <div className="content">
          ...
          <section className="automation-panel">
            ...
            <div className="automation-facts">
              ...
              <div><MessageSquare size={16} /><span>企业微信</span><strong>{automation?.notificationConfigured ? '已配置' : '待配置环境变量'}</strong></div>
            </div>
          ...
          <section className={`table-card ${denseTable ? 'compact-table' : ''}`}>
            ...
            <div className="filters-row">
              ...
              <div className="select-wrap"><select value={carrier} onChange={(event) => setCarrier(event.target.value)}><option>全部船司</option>{carriers.map((item) => <option key={item}>{item}</option>)}</select><ChevronDown size={15} /></div>
            ...
            <div className="table-scroll">
              <table>
                <thead><tr>
                  ...
                  <th>船司</th><th>到港时间<br/><span>ATA / ETA</span></th><th>提单号</th><th>柜号</th><th>卸船时间</th><th>船只状态</th><th>最后更新时间</th><th>备注</th><th>进度</th><th />
                </tr></thead>
                <tbody>
                  {loading ? <tr><td colSpan={11}><div className="loading-state"><LoaderCircle className="spin" />正在汇总船司数据…</div></td></tr> : filtered.length === 0 ? <tr><td colSpan={11}><div className="empty-state"><Search size={24} /><strong>没有匹配的船期记录</strong><span>调整关键词或筛选条件后再试</span></div></td></tr> : filtered.map((item) => (
                    <tr key={item.id} className={selected.has(item.id) ? 'selected-row' : ''}>
                      ...
                      <td><div className="carrier-cell"><CarrierMark code={item.carrierCode} /><div><strong>{item.carrier}</strong><span>{item.carrierCode}</span></div></div></td>
                      ...
                      <td><div className="update-cell"><span>{timeAgo(item.lastUpdated)}</span><small>{formatDateTime(item.lastUpdated)}</small></div></td>
                      <td><span className="note-cell" title={item.note}>{item.note || '—'}</span></td>
            ...
            <div className="table-footer"><span>已选择 {selected.size} 条</span><div><span>每页 20 条</span><button disabled>上一页</button><button className="page-active">1</button><button disabled>下一页</button></div></div>
          </section>
      ...
      {displaySettingsOpen && <div className="modal-backdrop settings-backdrop" role="presentation" onMouseDown={() => setDisplaySettingsOpen(false)}>
        <section className="settings-modal" role="dialog" aria-modal="true" aria-labelledby="display-settings-title" onMouseDown={(event) => event.stopPropagation()}>
          <div className="modal-heading"><div><p className="eyebrow">TABLE DISPLAY</p><h2 id="display-settings-title">显示设置</h2></div><button className="drawer-close" aria-label="关闭" onClick={() => setDisplaySettingsOpen(false)}><X size={19} /></button></div>
...
function ModulePage({ page, data, automation, syncing, onSync, onToggleAutomation, onCreateBackup, onRestoreBackup, onUpload, onToast, onOpenDetail }: {
  ...
  async function refreshModuleData() {
    ...
    try {
      ...
      if (page === 'sources') requests.push(fetch('/api/carriers').then((response) => response.json()).then((payload) => setCarrierRules(payload.carriers || [])));
      if (page === 'automation' || page === 'exports') requests.push(fetch('/api/automation/runs').then((response) => response.json()).then((payload) => setRuns(payload.runs || [])));
      if (page === 'exports') requests.push(fetch('/api/backups').then((response) => response.json()).then((payload) => setBackups(payload.backups || [])));
...
function SourcePill({ source }: { source: CarrierSource }) {
  return <div className="source-pill"><span className="source-color" style={{ background: source.color }} /><div><strong>{source.code}</strong><span>{source.recordCount} 条 · {timeAgo(source.lastSync)}</span></div><span className={`source-status ${source.status}`} /></div>;
}
